...
首页地图 ──悬停──► 预览卡 + 环境声
         ...
         ──声音实验室──► 融合播放 5 秒 ──► 填 ID ──► 随机纪念海报，可下载
```
...
声音实验室融合播放满 5 秒算「体验完成」：先停下两条音轨，再弹出 `components/keepsake.js`。用户填一个 ID，从 `data/keepsake.js`（目前 4 张：甲秀楼、青云市集、贵州省博物馆、黔灵山公园）随机抽一张，用 canvas 在图片下方补一条「票根」——印上用户 ID、日期、这次的融合组合与景点名，再 `toBlob` 出 jpg 供下载。票根按海报实际宽度除以 688 得到缩放系数排版，新增海报只要放进 `public/assets/images/keepsake/` 并登记数据，不必改绘制代码。重新播一次满 5 秒会再弹一次，抽屉底部的「领取声音纪念海报」按钮也常驻，随时能再抽。
